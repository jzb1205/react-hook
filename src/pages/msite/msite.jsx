...
function Msite(props){
  ...
  const [footTypes,setFootTypes] = useState([])
  const [title,setTitle] = useState('')
  const [text,setText] = useState("fff")
  const [imgBaseUrl,setimgBaseUrl] = useState("https://fuss10.elemecdn.com")
  ...
  // 根据经纬度获取地点信息
  async function getPoisSite(geohash){
    let res = await API.getPoisSite(geohash)
    setTitle(res.name)
  }
  async function cityGuess(){
    let res = await API.cityGuess();
    res.latitude = 118.170094294257
    res.longitude = 24.474107883956105
    setGeohash([res.latitude, res.longitude])
    props.saveAttrInfo('geohash', [res.latitude, res.longitude])
    await getPoisSite([res.latitude, res.longitude])
    await getFoodTypes();
  }
  function goHome(){
    props.history.push('/')
  ...
  return (
    <div>
      <Header title={props.route._title} signUp={true} goHome={goHome}/>
      <nav className="msite-nav">
        <div className="swiper-container">
          <div className="swiper-wrapper">
            {
              !footTypes.length &&<div className="food-types-container">
            ...
            {footTypes.map((foodItem, index) => {
              return (
                <div className="swiper-slide food-types-container" key={index}>
                  {foodItem.map((item, index) => {
                    return (
                      <Link className="link-to-food" to={`/food/${geohash}/${getCategoryId(item.link)}/${item.title}`} key={index}>
                        <figure>
                          <img
      ...
      <div className="shop-list-container">
        ...
        {footTypes.length?<ShopList geohash={geohash}/>:
      ...
      {!footTypes.length?<div className='site-loader'>
        <Loader/>
      </div>:''}
      <Footer/>
    </div>
